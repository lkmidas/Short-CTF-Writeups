...
mem += [ord('T')] + [1, 1, 1, 101, 1, 1, 1, 97, 1, 1, 1, 108, 1, 1, 1, 121, 1, 1, 1, 67, 1, 1, 1, 111, 1, 1, 1] + [0] * 0x1000 # input
...
def format_calc(typ, prec, width, pad, left, showsign, l, h, L, hh):
    addr = 0
    where = 0
    val = 0
    dbg_cmd = ""
    ...
    print("# addr = {}, val = {}".format(hex(addr), hex(val)))


def format_call(typ, prec, width, pad, left, showsign, l, h, L, hh):
    #global pc
...
def exec(init_pc):
    ...
    while True:
        ...
        if typ == "C":
            format_call(typ, prec, width, pad, left, showsign, l, h, L, hh)
        else:
            format_calc(typ, prec, width, pad, left, showsign, l, h, L, hh)
