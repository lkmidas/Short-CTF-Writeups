...
The `Launcher.exe` executable file throws an error when executed, then it seems like nothing happens after that. So I started to reverse it statically using IDA. The flow is quite simple: it first throws a fake error about missing a DLL, then cloning itself using the name `XblAuthenticator.exe`, then decrypt some data and save it under the name `XblCloud.dll`, all the files in created in this challenge are somehow related to XBox stuffs, but most likely they are just fake names. My teammates said that it also does something to the registry hive, but that information is not unecessary for the solution. The most important thing now is to get the DLL file. I think it can be retrieved by reversing the decryption function and decrypting the data, but my teammate `@Edisc` used `SpyStudio` and dumped it out for me. So all I need to do is to continue and analyze the DLL file and the traffic capture file.
...
- The user (or the keylogger, I didn't know yet) sends a zip file called `message.zip` containing `message.txt` to `c.unsafesector.com/upload` in TCP stream number 4. Dumping the zip file and trying to extract it, I found out that it is archived using an unknown password.
...
The next two `GetAsyncKeyState()` are called with parameters from `0x30 - 0x39` and `0x60 - 0x69`. Again looking at the documentation, these are number keys, with `0x30 - 0x39` being normal keys and `0x60 - 0x69` being numpad keys. They are then added with a constant to map into the range of `0x96 - 0x9f`.
...
Also in each of these encoding, I also saw that a timestamp retrieved from `GetTickCount64()` and a sequence number that got incremented after every logged key are included. There are also some other complex fields, but they are too complex to reverse, so I checked back at `Wireshark` to see if I could figure something out.
...
```
I could see there are two `0xcafe` bytes in this sequence, so this is likely to be the log of a mouse click. The first 3 bytes seems like a header of some sort. The fourth byte in these sequences gets incremented every sequence, starting from 0, so it must be the sequence number. The next 7 bytes are incremented a little bit every sequence, so I assumed that they are the timestamp. For the next 3 bytes, I don't even know what they are. And finally the last 8 bytes are the encrypted X and Y coordinate of the cursor when the mouse is pressed. This way, I could already parse the mouse click sequences into sequence number, timestamp and coordinates:
```python
def parse(data):
	length = u32(data[0:4])
	data = data[4:]
...
```
It seems that the dudes who are using this computer are just googling for some weird currency stuff on the Internet, nothing seems interesting to me yet. So I moved on to see what information I can gather from the last unknown part of the pcap: the big TCP stream number 10.
...
The lower part of this function is quite identical to the function that logs the keys, so it seemed like I went in the right direction. Scrolling up to the top of the function I found that it makes some calls to `keybd_event()` and some functions that interact with the clipboard. Some quick searches around the Microsoft docs and I knew that this function generate a `PrtScr` key press to take a screenshot and retrieve it from the clipboard. The image is then encoded in the `jpeg` format, I knew this by googling these 2 constants `1284378190221622446i64` and `3383081795586128797i64` in `sub_1800016B0`. Therefore, the big TCP stream is a `jpeg` image of the screenshot that got encrypted in some way.
...
v16 = v14->m128i_i64[0];
```
This looks like every byte in the image gets XORed with its next byte, except the last one. So I wrote a script to decrypt this to see if my theory is correct:
```python
from malduck import *
...
```
It actually is, so then I had the screenshot that got sent over by the keylogger.

## Analyze the screenshot
The screenshot was taken and sent after the user searched for the google calculator app, and did something with the numbers, as we can see from the parsed keylog. Linking it with the description, this is actually the 4 thieves stole some gold and then converted the value of the gold they stolen to VND and divided it by 4. 

At this point, because I saw the calculator and all the mouse clicks, I immediately thought of plotting all the mouse clicks onto the screenshot to find out what they did after calculating the money. All I found after that is disappointment, because the only thing that the "thieves" did after calculating the money is randomly clicking on the screen, then searched for `hero of the storm WTF` and watched some funny vids. It was a brainfart by me becasue I completely forgot about the `message.zip` file. It was even worse of a brainfart because I already thought of an On-screen keyboard shenanigans at this point, but somehow I couldn't link it with the password of the zip file.
...
for i in  range(16, 50):
	...
	cnt += 1
```
But because they also switched to the number keyboard and press some numbers, I have to take 2 screenshots and plot them separately:
```python
cnt = 1
...
for i in  range(37, 50):
	...
	cnt += 1
```
The password can be recovered as: `emergency password 641578642380`, but it still is incorrect. Therefore I asked the author of this challenge `@ks75vl` and he told me the there is actually one more key press before the first `e`, but I didn't found it (weird?), but I just used my instinct again and assume that it was a `Shift` key. So the correct password is: `Emergency password 641578642380`.
...
The script for decrypting the screenshot is at `decrypt_screenshot.py`.